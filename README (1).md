...
The site uses plain HTML and CSS with no framework, package manager, build command, JavaScript, or external font dependency. The four supplied PNG files are real page images referenced from `index.html`; keep the `assets` folder beside the HTML file.
...
The image paths are relative (`assets/jason-voorhees.png`, for example), which is the most reliable setup for GitHub Pages. Do not upload only `index.html`; the `assets` folder is part of the page.
...
The four character images were supplied for this project. Before publishing the repository publicly, confirm that you have the necessary permission or a valid license to redistribute them. To replace an image, overwrite the matching PNG in `assets` while keeping the same filename.
